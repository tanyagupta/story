...
Schema validation only proves that JSON has the expected shape. It does not prove that a record is a useful myth narrative. The bulk runner therefore applies semantic quality gates before a machine-generated record can be marked `approved`.

Approved record count is a semantic-quality measure, not a processing-volume measure.
...
The runner does not count generic `was`, `were`, or `had` statements as primary plot events, and it does not approve records with placeholder states such as `source-section`.
...
valid narrative candidates: 289
non-story candidates: 628
fully normalized records: 289
approved records: 24
records awaiting review: 265
open review items: probable duplicate and ambiguous family review queues
```

The approved count is currently below the desired target of 50 because the stricter deterministic gates reject or defer quote fragments, biographies, commentary, broad deity profiles, weak section headings, and profile-style deity/location/object headings instead of approving them to satisfy a volume target.

The normalized production records are deterministic automatic records. They preserve source provenance, candidate boundaries, evidence references, source-derived entity mappings, narrative summaries, source excerpts, and ordered source-supported events. The batch runner does not synthesize a canonical narrative across books; overlapping retellings are grouped by myth family and kept as distinct source variants.

Records generated by automation default to `awaiting_review`. A record is changed to `approved` only when all semantic gates pass. Records that are clearly not stories are marked as rejected non-story candidates in the catalog. Records with some narrative signals but failed gates remain available for review or correction.

Relationships are extracted only when the selected source wording clearly states one, such as parentage, marriage, assistance, pursuit, or enmity. Many approved records have no relationship entry because the selected passage supports actions and outcomes but does not explicitly state a relationship; absence is documented by semantic-quality notes instead of being filled from general mythology.
...
To inspect approved myths, start with `corpus/catalog/approved-myths.json`; each entry includes the normalized record path, main characters, synopsis, event count, and review status. Pending records are listed in `corpus/catalog/myths-awaiting-review.json` with failed semantic gates. Rejected non-story and poor-quality candidates are listed in `corpus/catalog/rejected-candidates.json`.
...
jq -s '{
  ...
  approved: ([.[] | select(.reviewStatus == "approved")] | length),
  awaiting_review: ([.[] | select(.reviewStatus == "awaiting_review")] | length),
  no_characters: ([.[] | select(.reviewStatus == "approved" and (((.entities.characters // []) | length) == 0))] | length),
  no_identified_event_actor: ([.[] | select(.reviewStatus == "approved" and (([.events[]? | select(.actor != null)] | length) == 0))] | length),
  placeholder_initial_state: ([.[] | select(.reviewStatus == "approved" and ((.initialState[0].subject // "") == "source-section"))] | length),
  no_storyline: ([.[] | select(.reviewStatus == "approved" and (((.narrative.storyline // .interpretation.storyline // []) | length) == 0))] | length),
  no_synopsis: ([.[] | select(.reviewStatus == "approved" and (((.narrative.synopsis // "") | length) == 0))] | length)
}' corpus/normalized/bulk/*.myth.json
```

The current deterministic extraction is still heuristic. It does not use a hosted model, it does not add unsupported facts from memory, and it may leave usable-looking but uncertain candidates in review when the source wording is too weak for automatic approval.
