...
split source proposals: 8
merged source proposals: 1
directly verified source proposals: 1
source-audited records generated by this branch: 17
derived or linked verified outputs accounting for selected proposals: 20
ambiguous records from this batch: 0
rejected non-story records: 0
unresolved records requiring human review from this batch: 0
remaining awaiting substantive source review: 248
```

The directly verified selected proposal is `bulk-myth-0044`, which becomes `bulk-verified-0016`, "Alcmaeon and the Necklace", reconstructed as a complete Berens section from ten contiguous passages. It rebuilds the machine proposal's sparse Apollo/Zeus extraction into source-supported entities, relationships, events, and narrative fields for Alcmaeon's revenge, punishment, deception, death, Calirrhoe's prayer, and the final deposition of Harmonia's necklace and veil at Delphi.

The repair pass completes the implementation work previously left as human review. Multi-episode source spans are split into source-bounded records where the passages justify it, and one duplicate Daedalus/Icarus proposal is merged into existing verified coverage rather than emitted as a duplicate story. Examples of derived records include "Jason Launches the Argo", "The Argonauts at Lemnos", "Birth and Exposure of Perseus", "Perseus Transforms Atlas", "Jason and the One Sandal", "Bellerophon Sent Against the Chimera", "Ulysses at Ismarus", "Ulysses and the Lotus-Eaters", and five source-bounded Argonautic subepisodes from the long travel sequence.

For split or merged proposals, the original machine proposal is marked superseded in the ledger and removed from active proposed catalogs. Provenance is retained through `derivedFromProposalIds`, `reconstructionBatch`, `derivationType`, and `sourceBoundaryDecision` on the verified records, plus the derived-record report. Existing verified records are linked when they already account for a selected proposal's source unit, such as Phryxus/Helle and Daedalus/Icarus.
